...
impl DeviceLogcatCommand {
    pub fn read_next_entry(tcp_stream: &mut TcpStream) -> Result<LogEntry, AdbError> {
        ...
        let pid = DeviceLogcatCommand::read_next_int32le(tcp_stream)?;
        let tid = DeviceLogcatCommand::read_next_int32le(tcp_stream)?;
        let sec = DeviceLogcatCommand::read_next_int32le(tcp_stream)?;
        let nsec = DeviceLogcatCommand::read_next_int32le(tcp_stream)?;
    ...
    fn read_commands() {
        ...
        match resp {
            AsyncDeviceProtocol::OKAY { mut tcp_stream } => loop {
                let entry = DeviceLogcatCommand::read_next_entry(&mut tcp_stream).unwrap();
