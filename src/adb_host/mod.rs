...
pub fn connect(connection_info: &HostConnectionInfo) -> Result<TcpStream, AdbError> {
    ...
    trace!("[connect]begin to create a new tcp connection: connection_str={}", connection_str.clone());
    let tcp_stream = match TcpStream::connect(connection_str.clone()) {
        | Ok(tcp_stream) => tcp_stream,
        | Err(error) => {
            debug!("[connect]create tcp connection failed: connection_str={}", connection_str.clone());
...
pub fn exec_command_sync(mut tcp_stream: TcpStream, command: String) -> Result<AsyncHostResponse, AdbError> {
...
pub fn exec_command(tcp_stream: &mut TcpStream, command: String) -> Result<SyncHostResponse, AdbError> {
...
pub fn read_response_content(tcp_stream: &mut TcpStream, length: usize) -> Result<String, AdbError> {
    let mut response_content = vec![0; length];
    match tcp_stream.read_exact(&mut response_content) {
        | Ok(_) => {}
        | Err(error) => {
            trace!("[read_response_content]read content failed: error={}", error);
            return Err(AdbError::TcpReadError {
                source: Box::new(error),
            });
        }
    ...
    match String::from_utf8(Vec::from(response_content)) {
        | Ok(content_string) => {
            trace!("[read_response_content]read command content success: content={}", &content_string);
            Ok(content_string)
        }
        | Err(error) => {
...
pub fn read_response_length(tcp_stream: &mut TcpStream) -> Result<usize, AdbError> {
    ...
    match String::from_utf8(Vec::from(content_length)) {
        | Ok(response) => {
            trace!("[read_response_length]read command content length success: length={}", &response);
...
pub fn read_response_status(tcp_stream: &mut TcpStream) -> Result<String, AdbError> {
    ...
    match tcp_stream.read_exact(&mut is_ok_buffer) {
        | Ok(_) => {}
        | Err(error) => {
            trace!("[read_response_status]read command status from stream failed: error={:?}", &error);
            return Err(AdbError::TcpReadError {
                source: Box::new(error),
            });
        }
